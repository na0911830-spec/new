...
export const ConfigSchema = z.object({
    server: z.object({
        ...
        secret_key: z.preprocess(
            (val) => {
                if (Deno.env.has("SERVER_SECRET_KEY")) {
                    return Deno.env.get("SERVER_SECRET_KEY") ?? "";
                }
                return val === undefined ? "" : val;
            },
            z.string()
                .length(16, { message: "String must contain exactly 16 characters" })
                .regex(/^[a-zA-Z0-9]+$/, { message: "SERVER_SECRET_KEY contains invalid characters" }),
        ).default(undefined),
